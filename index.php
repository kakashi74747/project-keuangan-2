...
// === DATA UNTUK GRAFIK ARUS KAS (LINE CHART) ===
...
// === DATA BARU UNTUK PIE CHART ALOKASI ASET ===
$asset_chart_labels = [];
$asset_chart_data = [];
...
// === DATA BARU UNTUK PIE CHART PENGELUARAN BULAN INI ===
$expense_chart_labels = [];
$expense_chart_data = [];
$query_expense_cat = mysqli_query($koneksi, "
    SELECT c.category_name, SUM(t.amount) as total 
    FROM transactions t 
    JOIN categories c ON t.category_id = c.id
    WHERE t.user_id = $user_id AND t.transaction_type = 'Pengeluaran' AND DATE_FORMAT(t.transaction_date, '%Y-%m') = '$bulan_ini'
    ...
    var myLineChart = new Chart(ctxArea, {
      ...
      options: {
        scales: {
          ...
          yAxes: [{ ticks: { min: 0, maxTicksLimit: 5, callback: formatRupiah }, gridLines: { color: "rgba(0, 0, 0, .125)" } }],
        },
        legend: { display: true },
    ...
    var myBarChartExpense = new Chart(ctxBarExpense, {
      ...
      options: {
        scales: {
          ...
          yAxes: [{ ticks: { min: 0, callback: formatRupiah }, gridLines: { display: true } }],
        },
